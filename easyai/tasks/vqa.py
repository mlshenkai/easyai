...
from easyai.common.utils import is_convertible_to_int
import easyai.common.dist_utils as dist_utils
...
class VQATaskWithTrainer(BaseTaskWithTrainer):
        ...
        for ds_name, dataset in datasets.items():
            for split in self.valid_splits:
                ...
                if (
                    ...
                    if (
                        split not in self.ques_files
                    ):  # precomputed and passed in task builder
                        self.ques_files[split] = os.path.join(
                            registry.get_path("cache_root"),
                            f"{ds_name}_gt",
                            f"{ds_name}_{split}_questions.json",
                        )
                        self.anno_files[split] = os.path.join(
                            registry.get_path("cache_root"),
                            f"{ds_name}_gt",
                            f"{ds_name}_{split}_annotations.json",
                        )
                        if dist_utils.get_rank() == 0:
                            os.makedirs(
                                os.path.join(
                                    registry.get_path("cache_root"), f"{ds_name}_gt"
                                ),
                                exist_ok=True,
                            )
                            try:
                                convert_to_coco_gt(
                                    dataset,
                                    self.ques_files[split],
                                    self.anno_files[split],
                                    split,
                                    self.sample_id_key,
                                )
                            except:
    ...
    def valid_step(self, model, samples):
        ...
        for answer, ques_id in zip(answers, question_id):
            ques_id = (
                int(ques_id.item()) if isinstance(ques_id, torch.Tensor) else ques_id
            )
...
def convert_to_coco_gt(
    data, outpath_questions, outpath_annotations, split, sample_id_key
):
    ...
    for ann in tqdm(data[split]):
        ...
        annotations_data["annotations"].append(
            {
                "question_type": ""
                if "question_type" not in ann
                else ann["question_type"],
                "multiple_choice_answer": ann["answers"][0]
                if isinstance(ann["answers"], list)
                else ann["answers"],
                "answers": [
                    {"answer": ans, "answer_id": i}
                    for i, ans in enumerate(ann["answers"])
                ]
                if isinstance(ann["answers"], list)
                else [{"answer": ann["answers"], "answer_id": 0}],
                "image_id": ann[sample_id_key],
                "question_id": ques_id,
