...
class CaptionTask(BaseTask):
        ...
        if self.annotation_file == None:
            if "coco" not in val_ds_name:  # coco is already precomputed in dataset
                self.annotation_file = os.path.join(
                    ...
                    f"{val_ds_name}_gt",
                    f"{val_ds_name}_{self.split}_annotations.json",
                )
                if get_rank() == 0:
                    os.makedirs(
                        os.path.join(
                            registry.get_path("cache_root"), f"{val_ds_name}_gt"
                        ),
                        exist_ok=True,
                    )
                    convert_to_coco_gt(
                        ...
                        self.sample_id_key,
                        self.split,
                        load_gt_from_file=self.load_gt_from_file,
    ...
    def valid_step(self, model, samples):
        ...
        for caption, img_id in zip(captions, img_ids):
            ...
            if (
                self.img_ids and img_id not in self.img_ids
            ):  # only include specified img_ids if specified
                continue
    ...
    def _report_metrics(self, eval_result_file, split_name):
        if self.annotation_file == None:
            ...
            coco_val = coco_caption_eval(
                coco_gt_root, eval_result_file, split_name, img_ids=self.img_ids
            )
        else:
            coco_val = coco_caption_eval(
                None,
...
def convert_to_coco_gt(
    ...
    if load_gt_from_file:
        print(
            f"Generating ground truth file for evaluation from {load_gt_from_file}...."
        )
        data = load_gt_file(load_gt_from_file)
        for ann in data:
            captions = ann[caption_key]
            img_id = (
                int(ann[sample_id_key])
                if is_convertible_to_int(ann[sample_id_key])
                else ann[sample_id_key]
            )
            if (
                img_ids and img_id not in img_ids
            ):  # only include specified img_ids if specified
                continue
            gt_data["images"].append({"id": img_id})
            if isinstance(captions, str):
                gt_data["annotations"].append(
                    {"image_id": img_id, "caption": captions, "id": img_id}
                )
            else:
                gt_data["annotations"].extend(
                    [{"image_id": img_id, "caption": c, "id": img_id} for c in captions]
                )
    else:
