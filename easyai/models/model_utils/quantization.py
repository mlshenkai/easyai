...
def configure_quantization(
    ...
    if getattr(config, "quantization_config", None):  # ptq
        if is_deepspeed_zero3_enabled():
            raise ValueError(
                "DeepSpeed ZeRO-3 is incompatible with PTQ-quantized models."
            )
        ...
        if quant_method == QuantizationMethod.AQLM:
            require_version(
                "transformers>=4.39.0", "To fix: pip install transformers>=4.39.0"
            )
        ...
        logger.info(
            "Quantizing model to {} bit.".format(model_args.export_quantization_bit)
        )
