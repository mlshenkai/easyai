...
from easyai.data.datasets.multimodal.caption_datasets import CaptionMultiModelDataset


class VideoCaptionDataset(CaptionMultiModelDataset):
...
class VideoCaptionEvalMultiModelDataset(BaseMultiModelDataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        """
        vis_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
        ...
        self.annotation = [
            ...
            if x["video"] not in seen and not seen.add(x["image_id"])
        ]
...
class ClipCaptionMultiModelDataset(BaseMultiModelDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        try:
            video = self.vis_processor(
                video_path,
                start_sec=math.floor(ann["ts"][0]),
                end_sec=math.ceil(ann["ts"][1]),
            )
        except:
            return None
...
class ClipCaptionInstructDataset(ClipCaptionMultiModelDataset):
    def __getitem__(self, index):
        data = super().__getitem__(index)
        if data != None:
            data["text_output"] = data["text_input"]
...
class ClipCaptionEvalDataset(ClipCaptionMultiModelDataset):
    def __getitem__(self, index):
        data = super().__getitem__(index)
        if data != None:
            del data["text_input"]
...
class WebVideoCaptionMultiModelDataset(BaseMultiModelDataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        super().__init__(vis_processor, text_processor, vis_root, ann_paths)
    ...
    def _get_video(self, index):
        ...
        for _ in range(max_retries):
            ...
            try:
                ...
                index = (index + 1) % len(
                    self.annotation
                )  # Safely loop back to start of annotations
        return None
...
class WebVideoCaptionInstructDataset(WebVideoCaptionMultiModelDataset):
    def __getitem__(self, index):
        data = super().__getitem__(index)
        if data != None:
            data["text_output"] = data["text_input"]
