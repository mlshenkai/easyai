...
from easyai.data.datasets.multimodal.dialogue_datasets import (
    DialogueMultiModelDataset,
    DialogueEvalMultiModelDataset,
)


class AVSDDialDataset(DialogueMultiModelDataset):
...
class AVSDDialEvalDataset(DialogueEvalMultiModelDataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        """
        vis_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
...
class AVSDDialInstructEvalDataset(DialogueMultiModelDataset):
    def __init__(self, **kwargs):
        ...
        self.annotation = [
            ann for ann in self.annotation if ann["image_id"] in self.sample_ids
        ]
        if "test" in kwargs["ann_paths"][0]:
            self.annotation = [
                ann for ann in self.annotation if ann["answer"] == "__UNDISCLOSED__"
            ]
