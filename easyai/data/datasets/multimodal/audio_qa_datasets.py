...
import copy
import os
from easyai.data.datasets.multimodal.audio_captioning_datasets import AudioCapsDataset
...
class ClothoQAMultiModelDataset(BaseMultiModelDataset):
    def __init__(self, **kwargs):
        super().__init__(
        ...
        for i in range(0, len(self.annotation), 3):
            ...
            if self.unanimous and Counter(new_ann["answer"])[new_ann["answer"][0]] != 3:
                continue
            if self.non_binary_cls and (
                "yes" in new_ann["answer"] or "no" in new_ann["answer"]
            ):
                continue
