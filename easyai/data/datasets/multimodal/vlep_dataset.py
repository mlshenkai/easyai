...
class VlepVideoMultiModelDataset(BaseMultiModelDataset):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        ...
        self.annotation = [
            ann for ann in self.annotation if ann["vid_name"] in existing_videos
        ]
...
class VlepVideoInstructDataset(VlepVideoMultiModelDataset):
    def __getitem__(self, index):
...
class VlepVideoEvalDataset(VlepVideoMultiModelDataset):
    def __getitem__(self, index):
        data = super().__getitem__(index)
        if data != None:
            del data["text_input"]
