...
class VALORCaptionMultiModelDataset(BaseMultiModelDataset):
    def __init__(self, **kwargs):
        ...
        self.annotation = [
            ...
            if ann["video_id"].replace("000", "0") in self.sample_ids
        ]
        seen = set()
        self.annotation = [
            x
            for x in self.annotation
            if x["video_id"] not in seen and not seen.add(x["video_id"])
        ]
    ...
    def get_audio_path(self, ann):
        return os.path.join(
            self.audio_root, f'{ann["video_id"].replace("000", "0")}.mp4'
        )

    def get_video_path(self, ann):
        return os.path.join(
            self.video_root, f'{ann["video_id"].replace("000", "0")}.mp4'
        )
...
class VALORCaptionEvalDataset(VALORCaptionMultiModelDataset):
    def __getitem__(self, index):
...
class VALORCaptionInstuctDataset(VALORCaptionMultiModelDataset):
    def __getitem__(self, index):
        data = super().__getitem__(index)
        if data != None:
