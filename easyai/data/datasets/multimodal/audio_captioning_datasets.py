...
class AudioCaptioningMultiModelDataset(BaseMultiModelDataset, __DisplMixin):
    def __init__(self, **kwargs):
        self.modalities = kwargs["modalities"]
        super().__init__(
            kwargs["vis_processor"],
            kwargs["text_processor"],
            kwargs["vis_root"],
            kwargs["ann_paths"],
        )
        for modality in self.modalities:
            ...
            setattr(
                self,
                f"existing_{modality}_annotation",
                getattr(self, f"get_existing_{modality}_annotations")(),
            )
    ...
    def get_video_path(self, ann):
        return pathlib.Path(
            os.path.join(self.video_root, ann[self.sample_id_key])
        ).resolve()

    def get_images_path(self, ann):
        return pathlib.Path(
            os.path.join(self.vis_root, ann[self.sample_id_key])
        ).resolve()
...
class AudioSetDataset(AudioCaptioningMultiModelDataset):
    def __init__(self, **kwargs):
        ...
        self.mid2label = {
            k: v for k, v in zip(df["mid"].tolist(), df["display_name"].tolist())
        }
        annotation = []
        ...
        self.sample_ids = set.intersection(
            *[
                set(getattr(self, f"existing_{modality}_annotation"))
                for modality in self.modalities
            ]
        )
        ...
        self.annotation = [
            ...
            if ann[self.sample_id_key] in self.sample_ids and ann[self.sample_id_key]
        ]
    ...
    def get_audio_path(self, ann):
        ...
        return str(
            os.path.realpath(
                os.path.join(
                    self.audio_root,
                    ann[self.sample_id_key]
                    + "_{:.1f}-{:.1f}.wav".format(
                        ann["start_seconds"], ann["end_seconds"]
                    ),
                )
            )
        ).replace("all_audio/", "")
    ...
    def __getitem__(self, index):
        ...
        if self.templates:
            ann["captions"] = [
                random.choice(self.templates).format(obj) for obj in objects
            ]
        else:
        ...
        for modality in self.modalities:
            ...
            if isinstance(ann[f"{modality}_path"], list):
                ...
                ann[modality if "image" not in modality else "image"] = getattr(
                    self, f"{'vis' if 'image' in modality else modality}_processor"
                )(ann[f"{modality}_path"])
...
class AudioCapsDataset(AudioCaptioningMultiModelDataset):
    def __init__(self, **kwargs):
        self.sample_id_key = "youtube_id"
        self.split = (
            "train"
            if "train" in kwargs["ann_paths"][0]
            else "test"
        ...
        self.modalities = kwargs["modalities"]
        for modality in self.modalities:
            kwargs[f"{modality}_root"] = os.path.join(
                kwargs[f"{modality}_root"], f"{self.split}"
            )
        super().__init__(**kwargs)
        ...
        def get_existing_audio_annotations(self):
            return (
                [f.split("_")[0] for f in os.listdir(self.audio_root)]
                if not self.cached
                else [f.split("_")[0] for f in os.listdir(self.cached_dir)]
            )
        ...
        self.annotation = [
            ann
            for ann in self.annotation
            if ann[self.sample_id_key] in self.sample_ids
            and ann[self.sample_id_key] not in kwargs.get("missing_ids", [])
        ]
        self._add_instance_ids()
        print(f"Loaded {len(self.annotation)} examples.")
    ...
    def get_audio_path(self, ann):
        ...
        return os.path.join(
            self.audio_root,
            ann[self.sample_id_key] + "_{}.flac".format(int(ann["start_seconds"])),
        )
    ...
    def get_cached_audio_path(self, ann):
        ...
        return os.path.join(
            self.cache_dir,
            ann[self.sample_id_key] + "_{}.flac.pt".format(int(ann["start_seconds"])),
        )
    ...
    def __getitem__(self, index):
        ...
        for modality in self.modalities:
            if modality == "audio" and self.cached:
                ann[f"{modality}_path"] = getattr(self, f"get_cached_{modality}_path")(
                    ann
                )
...
class AudioCapsEvalDataset(AudioCapsDataset):
    def __init__(self, **kwargs):
        ...
        self.annotation = [
            ...
            if x["youtube_id"] not in seen and not seen.add(x["youtube_id"])
        ]
...
class ClothoV2MultiModelDataset(BaseMultiModelDataset, __DisplMixin):
    def __init__(self, **kwargs):
        super().__init__(
            kwargs["vis_processor"],
            kwargs["text_processor"],
    ...
    def __getitem__(self, index):
        ann = copy.deepcopy(self.annotation[index])
        ann["audio"] = self.audio_processor(
            os.path.join(self.audio_root, self.split, ann["fname"])
        )
        if ann["audio"].sum() == 0:
            return None
...
class ClothoV2InstructDataset(ClothoV2MultiModelDataset):
    def __getitem__(self, index):
        data = super().__getitem__(index)
        if data != None:
            data["text_output"] = data["text_input"]
...
class ClothoV2EvalDataset(ClothoV2MultiModelDataset):
    def __getitem__(self, index):
        data = super().__getitem__(index)
        if data != None:
            del data["text_input"]
...
class AudioLanguagePretrainMultiModelDataset(BaseMultiModelDataset, __DisplMixin):
    def __init__(self, **kwargs):
    ...
    def __getitem__(self, index):
        ...
        if self.cached:
            audio = torch.load(
                os.path.join(self.cache_dir, f"{ann['id']}.pt"),
...
class AudioLanguagePretrainInstructDataset(AudioLanguagePretrainMultiModelDataset):
    def __getitem__(self, index):
        data = super().__getitem__(index)
        if data != None:
            data["text_output"] = data["text_input"]
...
class AudioLanguagePretrainEvalDataset(AudioLanguagePretrainMultiModelDataset):
    def __getitem__(self, index):
        data = super().__getitem__(index)
        if data != None:
            del data["text_input"]
