...
class ModelNetClassificationMultiModelDataset(BaseMultiModelDataset, __DisplMixin):
    """
    ...
    def __init__(self, **kwargs):
        super().__init__(
            kwargs["vis_processor"], kwargs["text_processor"], kwargs["vis_root"], []
        )
        ...
        assert (
            "pc_root" in kwargs
        ), "Point cloud root needs to be provided to retrieve labels."
        ...
        self.datapath = [
            (
                self.shape_names[i],
                os.path.join(self.pc_root, self.shape_names[i], self.shape_ids[i])
                + ".txt",
            )
        ...
        self.save_path = (
            ann_paths[1] if self.uniform else ann_paths[0].replace("_fps", "")
        )
