...
class YT8MDialMultiModelDataset(BaseMultiModelDataset):
    def __init__(self, **kwargs):
        ...
        self.annotation = [
            ann for ann in self.annotation if ann["youtube_id"] in self.sample_ids
        ]
    ...
    def get_audio_path(self, ann):
        return os.path.join(
            self.audio_root,
            f'{ann["youtube_id"]}_{ann["start_sec"]}_{ann["end_sec"]}.flac',
        )

    def get_video_path(self, ann):
        return os.path.join(
            self.video_root,
            f'{ann["youtube_id"]}_{ann["start_sec"]}_{ann["end_sec"]}.mp4',
        )
    ...
    def __getitem__(self, index):
        ...
        for modality in self.modalities:
            ...
            if "video" in modality:
                try:
                    ann["video"] = getattr(self, f"video_processor")(
                        ann[f"video_path"],
