...
class __DisplMixin:
    def displ_item(self, index):
        ...
        return OrderedDict(display)


class DisCRnMultiModelDataset(BaseMultiModelDataset, __DisplMixin):
    def __init__(self, **kwargs):
        ...
        self.annotation = [
            ...
            if ann["sample_ids"][0] in self.sample_ids
        ...
        if self.ds_name == "objaverse":
            return [
                ...
                for f in os.listdir(
                    os.path.join(
                        self.vis_root,
                        f"compressed_imgs_view{self.view}/Cap3D_imgs_view{self.view}/",
                    )
                )
            ]
    ...
    def get_image_path(self, ann, entity_index):
        if self.ds_name == "objaverse":
            ...
            return os.path.join(
                self.vis_root,
                f"compressed_imgs_view{self.view}/Cap3D_imgs_view{self.view}/",
                ann["sample_ids"][entity_index] + f"_{self.view}.jpeg",
            )
    ...
    def get_audio_path(self, ann, entity_index):
        if self.ds_name == "audiocaps":
            return str(
                os.path.join(
                    self.audio_root,
                    ann["sample_ids"][entity_index]
                    + "_{}.flac".format(int(ann["start_seconds"][entity_index])),
                )
            )
            ...
            return str(
                os.path.realpath(
                    os.path.join(
                        self.video_root,
                        ann["sample_ids"][entity_index]
                        + "_{}.mp4".format(int(ann["start_seconds"][entity_index])),
                    )
                )
            )
    ...
    def get_pc_path(self, ann, entity_index):
        if self.ds_name == "objaverse":
            return os.path.join(
                self.pc_root,
                ann["sample_ids"][entity_index],
                "{}_{}.npz".format(ann["sample_ids"][entity_index], self.npoints),
            )
    ...
    def __getitem__(self, index):
        ...
        for i, modality in enumerate(self.modalities):
            if (
                ann[f"captions_pred_{modality}"] == None
                or ann[f"captions_pred_{modality}"][i] == None
            ):
                return None
        ...
        if self.balance_labels:
            if (index % 2 and ann["label"] == 1) or (
                not index % 2 and ann["label"] == 0
            ):
                ...
                if self.shuffle_modalities:
                    ann["modalities"] = [
                        ann["modalities"][1],
                        ann["modalities"][0],
                    ]  # if we comment this out, we can have batch size > 1. Maintaining for reproducibility.
                for modality in self.modalities:
                    ann[f"captions_pred_{modality}"] = [
                        ann[f"captions_pred_{modality}"][1],
                        ann[f"captions_pred_{modality}"][0],
                    ]

        ## baseline captions
        ann["baseline_captions"] = (
            [c for c in ann["captions"]]
            if self.ground_truth
            else [
                ann[f'captions_pred_{ann["modalities"][0]}'][0],
                ann[f'captions_pred_{ann["modalities"][1]}'][1],
            ]
        )
        # ann["baseline_captions"] = [c.replace('..', '.') for c in ann["baseline_captions"]]
        ann["baseline_captions"] = [
            c.strip() if c != None else "" for c in ann["baseline_captions"]
        ]
        ## text input
        ann["text_input"] = self.text_processor(
            f'{ann["question"].replace("which entity", "which of the two options").replace("which object", "which of the two options").replace("which image", "which of the two options").replace("which audio", "which of the two options").replace("audio", "object").replace("image", "object")}?'.replace(
                "??", "?"
            )
        )
        ...
        if "pc" in ann["answers"]:
            ann["answers"].extend(
                ["3d", "3d model", "model", "rendering", "a 3d", "a 3d model"]
            )
