...
class Object3DCaptionMultiModelDataset(BaseMultiModelDataset, __DisplMixin):
    def __init__(self, **kwargs):
        ...
        self.annotation = [
            ann for ann in self.annotation if ann["sample_id"] in self.sample_ids
        ]
    ...
    def __getitem__(self, index):
        ...
        for modality in self.modalities:
            ann[f"{modality}_path"] = getattr(self, f"get_{modality}_path")(
                ann["sample_id"]
            )
            ...
            if "image" in modality:
                ...
                ann[modality] = getattr(self, f"{modality}_processor")(
...
class ObjaverseCaptionDataset(Object3DCaptionMultiModelDataset, __DisplMixin):
    def __init__(self, **kwargs):
    ...
    def get_existing_images_annotations(self):
        return [
            ...
            for f in os.listdir(
                os.path.join(
                    self.vis_root, f"compressed_imgs_view{0}/Cap3D_imgs_view{0}/"
                )
            )
        ]
    ...
    def get_existing_pc_annotations(self):
        return list(
            set(os.listdir(self.pc_root)).intersection(
                set(ann["sample_id"] for ann in self.annotation)
            )
        )

    def get_pc_path(self, sample_key):
        return os.path.join(
            self.pc_root, sample_key, "{}_{}.npz".format(sample_key, self.npoints)
        )
    ...
    def get_images_path(self, sample_key):
        ...
        return [
            os.path.join(
                self.vis_root,
                f"compressed_imgs_view{i}/Cap3D_imgs_view{i}/",
                sample_key + f"_{i}.jpeg",
            )
            for i in range(8)
        ]
...
class ShapenetCaptionDataset(Object3DCaptionMultiModelDataset, __DisplMixin):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def get_images_path(self, sample_key):
        return [
            os.path.join(self.vis_root, sample_key, img_path)
            for img_path in os.listdir(os.path.join(self.vis_root, sample_key))
        ]
    ...
    def __getitem__(self, index):
        ...
        if not isinstance(ann["captions"], list):
            if self.templates:
                ...
                ann["captions"] = [
                    random.choice(self.templates).format(obj)
                    for obj in ann["objects"].split(",")
                ]
            else:
                ann["objects"] = ann["captions"]
